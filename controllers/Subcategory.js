...
async function getRecord(req, res) {
    ...
    try {
        let data = await Subcategory.find().sort({ _id: -1 })  //sort data in descending order
...
async function createRecord(req, res) {
    ...
    try {
        ...
        console.log(error);
        if (error.keyValue)
            res.send({ status: 400, result: "Failed", message: "SubCategory Must Be Unique" })
...
async function deleteData(req, res) {
    try {
        ...
        if (data) {
            ...
            res.send({ status: 200, result: "Data Deleted", message: "Data Deleted Successfully" })
        }
        else {
            res.send({ status: 404, result: "Failed", message: "Data Not Found" })
        }
    } catch (error) {
