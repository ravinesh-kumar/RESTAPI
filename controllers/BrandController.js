//Model call kro
// http://localhost:8080/api/user/forgetpassword1
...
async function getRecord(req, res) {
    ...
    try {
        let data = await Brand.find().sort({ _id: -1 })  //sort data in descending order
...
async function createRecord(req, res) {
    ...
    try {
        ...
        console.log(error);
        if (error.keyValue)
            res.send({ status: 400, result: "Failed", message: "Brand Must Be Unique" })
